...
main() {
  ...
  DEVICE=$(adb devices | awk '/\tdevice$/{print $1}' || true)
  if [[ -z "$DEVICE" ]]; then
      warn "No Android device connected. Enable USB debugging."
      exit 1
  fi
  ...
  log_info "Screen Size         : $(adb shell wm size 2>/dev/null | awk '{print $3}')"
  log_info "Battery Level       : $(adb shell dumpsys battery | awk '/level/ {print $2}')%"
  log_info "Charging Status     : $(adb shell dumpsys battery | awk -F': ' '/status/ {print $2}' | xargs)"
  log_info "Uptime              : $(adb shell uptime -p 2>/dev/null | tr -d '\r')"
  log_info "User(s)             : $(adb shell pm list users | grep 'UserInfo' || echo "(none)")" 
  log_info "Root Access         : $(adb shell su -c id 2>/dev/null || echo "No root access")"
  log_info "Storage info          : $(adb shell df || warn "Could not retrieve storage info")"  
  log_info "Apps System:"
  adb shell pm list packages -s | cut -d: -f2 | sort | sed 's/^/System ➤ /'

  log_info "Apps User:"
  adb shell pm list packages -3 | cut -d: -f2 | sort | sed 's/^/User ➤ /'
