...
# Description : NETWORK  - Scan Hosts up on a network target 
...
print_banner() {
  echo "==========================================================="
  echo -e "🛡️  ${CYAN}NETOPSYS${RESET} - NETWORK - Scan Hosts up on a network target "
  echo "==========================================================="
}
...
warning_script() {
  echo -e "${YELLOW}[!] Responsability warning${RESET}"
  echo -e "--------------------------"
...
scan_hosts_up() {
  ...
  ip neigh flush all 
  sleep 4 
  arp-scan -I "$INTERFACE" -g "$NETWORK" -q | grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}' | grep -v "$MY_IP" | tee "$LOG_FILE"
}
