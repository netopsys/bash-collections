...
# Description : SYSTEM   - Scan files metadata size limit
...
print_banner() { 
  echo "==========================================================="
  echo -e "🛡️  ${CYAN}NETOPSYS${RESET} - SYSTEM - Scan files metadata size limit"
  echo "===========================================================" 
}
...
check_limits() {
  ...
  for file in "${files[@]}"; do
    ...
    if (( size > limit )); then
      log_warn "$sz_human > $limit_human | $file"
    else
      # log_ok "$ext | $sz_human <= $limit_human | $file"
      log_ok "$sz_human $file"
    fi
  done
  log_info "Total files found: ${#files[@]}"
