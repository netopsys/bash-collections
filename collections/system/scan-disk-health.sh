...
# Description : SYSTEM   - Scan Disk health infos
...
print_banner() { 
  echo "==========================================================="
  echo -e "🛡️  ${CYAN}NETOPSYS${RESET} - SYSTEM - Scan Disk health infos"
  echo "==========================================================="  
} 
...
analyze_disk() {
  ...
  # udevadm
  # udevadm info --query=all --name="$DISK_PATH"

  # Partition table
  log_info "=== TEST - PARTITION TABLE - $DISK_PATH ==="
  local PART_INFO
  PART_INFO=$(parted -s "$DISK_PATH" print 2>&1)
  if echo "$PART_INFO" | grep -q "unrecognised disk label"; then
    log_info "$PART_INFO"
    log_warn "No partition table detected."
  else
    log_info "$PART_INFO"
  ...
  # hdparm security
  log_info "=== TEST - HDPARM - $DISK_PATH ==="
  local SECINFO
  ...
  if check_disk_security "$SECINFO"; then
    ...
    log_info "Actuel state $SECINFO"
    log_warn "Disk $DISK_PATH is not in an ideal security state."
  fi

  # SMART test
  log_info "=== TEST - S.M.A.R.T - $DISK_PATH ==="
  if smartctl -i "$DISK_PATH" &> /dev/null; then
    ...
    if [[ "$smart_status" == "PASSED" ]]; then
      ...
      log_warn "SMART status: ISSUE ($smart_status)"
    fi
  else
    log_warn "SMART not supported on $DISK_PATH"
  fi

  # Disk space (only /dev/sda)
  if [ "$DISK_PATH" == "/dev/sda" ]; then
    log_info " === TEST - DISK SPACE USAGE - $DISK_PATH ==="
    local used_pct
    used_pct=$(df / | awk 'NR==2 {print $5}' | tr -d '%')
    if [[ used_pct -ge 90 ]]; then
      log_ok "Used space: ${used_pct}%"
    else
      log_warn "Disk is almost full! (${used_pct}%)"
    fi 
  fi

  # Bad blocks
  log_info "=== TEST - BAD BLOCKS - $DISK_PATH ==="
  read -rp "➤ Scan for bad blocks (read-only, slow)? [y/N] : " CHOICE
  if [[ "${CHOICE,,}" == "y" ]]; then
    local LOG="/var/log/badblocks_disk_${DISK}.log"
